...
print(f"LINE_CHANNEL_ACCESS_TOKEN loaded (length: {len(LINE_CHANNEL_ACCESS_TOKEN)}) If this is 'YOUR_CHANNEL_ACCESS_TOKEN', please set your environment variables.")
print(f"LINE_CHANNEL_SECRET loaded (length: {len(LINE_CHANNEL_SECRET)}) If this is 'YOUR_CHANNEL_SECRET', please set your environment variables.")
...
def callback():
    ...
    try:
        ...
        print("Invalid signature. Request body: " + body)
        abort(400)
...
def handle_image_message(event):
    print("進入 handle_image_message 函數")
    ...
    try:
        ...
        try:
            ...
            print(f"解析 recognition_result 失敗: {e}")
            # 如果解析失敗，使用預設值
            veg_name = "未知蔬菜"
            confidence = 0.0
        ...
        vegetable_details = get_vegetables_by_name_or_alias(
            ...
            nutrition_file_path='veg_data/vege_nutrition_202507211504.xlsx',
            basic_vege_file_path='veg_data/basic_vege_202507211504_good.xlsx',
            alias_file_path='veg_data/vege_alias_202507211504.xlsx'
        )
        ...
        print("Image recognition reply sent successfully.")
    except Exception as e:
        print(f"Error processing image or sending reply: {e}")
        messaging_api.reply_message(
            ReplyMessageRequest(
                reply_token=event.reply_token,
        ...
        if os.path.exists(image_filename):
            ...
            print(f"Deleted temporary image file: {image_filename}")
